...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get batch details including warehouse_id
    const [batchRows] = await connection.execute(`
      SELECT batch_id, created_date, status, warehouse_id FROM disposal_batch WHERE batch_id = ?
    `, [batch_id]);
    ...
    const warehouse_id = batch.warehouse_id;
    ...
    // Get all pharmacies for requests in this batch (just need pharmacy_id from request, then join to pharmacy table)
    const [pharmacyRows] = await connection.execute(`
      ...
      WHERE pdr.batch_id = ?
      ORDER BY pdr.request_id
    `, [batch_id]);

    await connection.end();

    if (!pharmacyRows || pharmacyRows.length === 0) {
      return res.status(404).json({ error: "No pharmacies found for batch" });
    }
    ...
    console.error("Error calculating disposal batch route:", error);
